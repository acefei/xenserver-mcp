...
- **File Operations**: `editFiles`, `problems`
...
Reference tools in the prompt body using `#tool:<tool-name>` syntax (e.g., `#tool:githubRepo`).

### 8. **Technical Configuration**
- Should this run with a specific agent? (`agent` (default), `ask`, `edit`, or custom agent name)
- Does it require a specific model? (If not specified, the currently selected model in model picker is used)
- Does it need an `argument-hint` to guide users on how to interact with the prompt?
...
name: "[Optional: name for /command, defaults to filename]"
argument-hint: "[Optional: hint text shown in chat input field]"
agent: "[agent|ask|edit or custom agent name - defaults to 'agent']"
tools: ["[appropriate tools based on functionality]"]
model: "[optional: only if specific model required]"
---
...
- Use `${selection}` or `${selectedText}` for selected code
- Use `${file}`, `${fileBasename}`, `${fileDirname}`, `${fileBasenameNoExtension}` for file context
- Use `${workspaceFolder}`, `${workspaceFolderBasename}` for workspace context
- Use `${input:variableName}` or `${input:variableName:placeholder}` for user input
- Reference other files using relative Markdown links
